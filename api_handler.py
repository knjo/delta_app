...
class Wos_reply:
    ...
    def text_list(self) :
        ...
        for i in range(len(self.wos_list)):
            time = "日期:" + str(self.wos_list[i]['startTime']).split('T')[0] + " " + str(self.wos_list[i]['startTime']).split('T')[1].split('.')[0] +" ~ " + str(self.wos_list[i]['endTime']).split('T')[0] + " " + str(self.wos_list[i]['endTime']).split('T')[1].split('.')[0]
            name = "機台:" + str(self.wos_list[i]['eqpName']) + "  工單:"+ str(self.wos_list[i]['woName'])+ "  產品:"  +str(self.wos_list[i]['itemName']) +"  資料筆數:" + str(self.wos_list[i]['orderQty']) 
            text = {"name" : name , "index" : i , "time" : time}
            wos_list.append(text)
        self.wos_list_html = wos_list
        self.sampleName = str(self.wos_list[i]['eqpName']) + "_"+ str(self.wos_list[i]['woName'])+ "_"  +str(self.wos_list[i]['itemName'])+ str(datetime.today()).split(" ")[0]
